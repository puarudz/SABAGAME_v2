$(document).ready(function () {
    ...
    // auto scroll to bottom game logs
    window.setInterval(function () {
        var elem = document.getElementById('logs');
        elem.scrollTop = elem.scrollHeight;
    }, 300);

    // tính trước vốn cần để bet
...
ws.onopen = function () {
    ...
    $(document).ready(function () {
        ...
        $('#excute').click(function () {
            ...
            var bridge_input = $("#bridge_input");
            if (!bridge_input.val()) {
                cuteToast({
                    type: 'error',
                    message: 'Bạn Chưa Chọn Bet',
                    timer: 5000
                });
                return;
            }
            ...
            var bridge = bridge_input.val().substring(0, bridge_input.val().length - 1);
            var arr_logic = [];
            var split_bridge = bridge.split(",");
            $.each(split_bridge, function (i, val) {
                arr_logic.push(val);
            });

            var autowork = $('#autowork').prop('checked');
...
ws.onmessage = function (data) {
    ...
    switch (_data.type) {
        ...
        case 'analytic':
            ...
            if (_data.data.action == 'list_result') {
                var list_result = _data.data.msg;
                $('#list_result').html('');
                $.each(list_result, function (i, val) {

                    if (val == 1) {
                        $('#list_result').append('<img src="images/tai-dot.png" class="bet-dot">');
                    } else if (val == 2) {
                        $('#list_result').append('<img src="images/xiu-dot.png" class="bet-dot">');
                    }

                });
            }

            if (_data.data.action == 'list_compare') {
                var list_compare = _data.data.msg;
                //list_compare.sort().reverse();
                $('#list_compare').html('');
                $.each(list_compare, function (i, val) {
                    if (val == 1) {
                        $('#list_compare').append('<img src="images/tai-dot.png" class="bet-dot">');
                    } else if (val == 2) {
                        $('#list_compare').append('<img src="images/xiu-dot.png" class="bet-dot">');
                    }
                });
            }
...
function add_logic(type) {
    var bridge_input = $("#bridge_input");
    bridge_input.val(bridge_input.val() + type + ',');

    switch (type) {
        case '1':
            $('#value_bridge').append('<img src="images/tai-dot.png" class="bet-dot">');
            break;
        case '2':
            $('#value_bridge').append('<img src="images/xiu-dot.png" class="bet-dot">');
            break;
    }
}
